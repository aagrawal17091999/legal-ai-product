...
export default function OcrViewerPage({
  ...
  const [view, setView] = useState<{
    ...
    pdfUrl: string | null;
    docxUrl: string | null;
  }>({ loaded: false, notFound: false, job: null, result: null, pdfUrl: null, docxUrl: null });
  ...
  const load = useCallback(async () => {
    ...
    if (res.status === 404) {
      setView({ loaded: true, notFound: true, job: null, result: null, pdfUrl: null, docxUrl: null });
      return;
    }
    if (!res.ok) {
  ...
  const { loaded, notFound, job, result, pdfUrl, docxUrl } = view;
  ...
  useJobStatusPush("ocr", job?.status === "processing" ? [id] : [], load);
    ...
    <div className="flex-1 overflow-y-auto">
      <div className="max-w-3xl mx-auto px-6 py-10">
        ...
        {!loaded ? (
          ...
          <p className="mt-8 text-[14px] text-charcoal-500">Document not found.</p>
        ) : (
          <>
            {/* ── Header ── */}
            ...
            {job.status === "processing" && (
              <div className="mt-8 flex items-center gap-2 text-[13px] text-gold-700">
                <Spinner size="sm" /> Running OCR… this page updates automatically.
              </div>
            )}
              ...
              <>
                <div className="mt-6 rounded-lg border border-gold-400 bg-gold-100/50 px-4 py-3">
                  <p className="text-[13px] text-charcoal-700 leading-relaxed">
                    <span className="font-semibold">Draft for review.</span> This is an automated
                    transcription. Sections the system could not read confidently are flagged below
                    rather than guessed — review before relying on it.
                  </p>
                </div>
                ...
                {result ? (
                  <>
                    {result.flaggedCount > 0 && (
                      <p className="mt-5 text-[13px] font-medium text-burgundy-700">
                        ...
                        system could not read or render them with confidence.
                      </p>
                    )}
                  ...
                  <p className="mt-6 text-[13px] text-charcoal-500">
                    An in-app preview isn&apos;t available for this older document. Use the download
                    buttons above to open it.
                  </p>
                )}
              </>
            )}
...
function Segment({ seg }: { seg: TranscribedSegment }) {
